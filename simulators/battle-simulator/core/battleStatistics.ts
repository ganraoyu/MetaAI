...
const calculateAllDamageDelt = async (
  ...
  try {
    ...
    const allOpponentDamage = totalOpponentDamage.map((champion: DamageData, index: number) => ({
      name: champion.name,
      starLevel: opponentStatistics[index]?.starLevel || 1,
      totalAttackDamage: champion.totalDamage,
...
const calculateAllBattleStatistics = async (
  ...
  try {
    ...
    const { checkPlayerChampionAliveOrDead, checkOpponentChampionAliveOrDead } =
      calculateIsAliveOrDeadInternal();
