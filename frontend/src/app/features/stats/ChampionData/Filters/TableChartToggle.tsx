...
export const TableChartToggle = () => {
  ...
  return (
    <div className="flex flex-col">
      <div 
      className={`flex items-center justify-center w-6 h-6  cursor-pointer rounded-md ${table ? 'bg-[#2d2d2d]' : 'bg-transparent'}`}
        onClick={() => {setTable(true); setChart(false);}}
      >
        <FontAwesomeIcon icon={faTable} className='w-3 h-3'/>
      </div>
      <div className={`flex items-center justify-center w-6 h-6 cursor-pointer rounded-md ${chart ? 'bg-[#2d2d2d]' : 'bg-transparent'}`}
