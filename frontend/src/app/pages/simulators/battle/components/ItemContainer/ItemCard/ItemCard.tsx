...
export const ItemCard = ({ itemTypeArray }: ItemCardProps) => {
  ...
  const filteredItems = itemTypeArray.filter((item) =>
  ...
  const handleDragStart = (e: React.DragEvent, itemName: string) => {
    e.dataTransfer.setData(
      "application/json",
      JSON.stringify({ name: itemName, type: "item" })
    );
    ...
    <div className="flex flex-wrap gap-2 m-1">
      {filteredItems.map((item, index) => (
        <div
          ...
          onDrag={(e) => {e.currentTarget.classList.add("cursor-grabbing");}}
        >
          <img src={item.image} alt={item.name} className="w-[2rem] h-[2rem] cursor-grab active:cursor-grabbing" draggable={true}  />
          <p
            className="text-[0.5rem] truncate w-full text-center"
            title={item.name}
          >
            {item.name}
          </p>
