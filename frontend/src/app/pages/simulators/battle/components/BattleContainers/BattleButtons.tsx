...
export const BattleButtons = () => {
  ...
  const handleNext = () => {
    if (battleHistory && battleHistory.battleLogs && currentBattleIndex < battleHistory.battleLogs.length - 1) {
      setCurrentBattleIndex(currentBattleIndex + 1);
    }
  };
    ...
    <div>
      <div className='flex items-center gap-1 mb-[0.5rem]'>
        ...
        {battleHistory && battleHistory.battleLogs && battleHistory.battleLogs.length > 0 && (
          <>
            <button 
              className={`${iconButtonClass} ${currentBattleIndex <= 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
            ...
            <button 
              className={`${iconButtonClass} ${currentBattleIndex >= battleHistory.battleLogs.length - 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
              onClick={handleNext}
        ...
        {/* Analytics button */}
        <button 
          className={`${iconButtonClass} ${showAnalytics ? 'border-blue-500/60 text-blue-400' : ''}`}
