...
export const HexCell: React.FC<HexCellProps> = ({ cellId }) => {
  ...
  return (
    <div
      ...
      title={champion ? `${champion} (Double-click to remove)` : "Drop champion here"}
    >
      {/* Outer hexagon (border) */}
      <div
        className="absolute inset-0 shadow-md"
        style={{
          border: championData ? `1px solid ${getBorderColor(championData.cost)}` : "none",
        ...
        >
          {[1, 2, 3].map((star) => (
            ...
            >
              <Star
                textColor={"#FFFF00"}
                fillColor={star <= (championData.starLevel || 1) ? "#FFFF00" : "transparent"}
              />
            </div>
          ))}
        </div>
      ...
      <div className="w-full absolute bottom-0 left-0">
        {championItem?.map((item, index) => {
          ...
          return (
            <div
              key={item}
              className={`absolute ${positions} border border-yellow-300 z-[51] hover:cursor-pointer`}
              onMouseDown={() => removeItemFromChampion(cellId, item)}
            >
